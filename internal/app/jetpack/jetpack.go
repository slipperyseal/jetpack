...
var storeMap [0xffff]bool
var loadMap [0xffff]bool
var memoryMap [0xffff]uint16 // compacted relocated addresses. this might not work
...
func nextSetsNZ() bool {
	return InstructionsSetNV[data[pc-header.LoadAddress]]
}
...
func checkSelfMod(addr uint16) {
	if codeMap[addr] {
		log.Fatal(fmt.Sprintf("WRITE TO CODE BLOCK $%04x - PC $%04x\n", addr, pc))
	}
}
...
func listCodeBlocks() {
	var lower = header.LoadAddress
	var upper = header.LoadAddress + uint16(len(data))
	change := codeMap[0]
	var movedMem uint16 = 0
	var index uint16
	for index = 0; index < 0xffff; index++ {
		code := codeMap[index]

		// compact data areas
		if index >= lower && index <= upper && !code {
			memoryMap[index] = movedMem
			movedMem++
		}
